...
class EventScraper:
    ...
    def team_sport(self, urls, filename, columns):
        ...
        table_csv = pd.DataFrame(result, columns=['Main event name','match type', 'match name', 'score type', "date", "time", "location"]+columns)
    ...
    def sports(self, urls, filename, columns):
                ...
                for row in rows:
                    if len(event_details) <= 0:
                        ...
                        for score_row in scores:
                            ...
                            sub_values.insert(0, match_type)    
                            values = [main_event_name] + sub_values
                            values[1] = values[1].replace("\xa0","")
                            values[7] = values[7].replace("\xa0","")
                            result.append(values)
                        event_details = []
        table_csv = pd.DataFrame(result, columns=['Main event name', 'match type', 'match name', "date", "time", "location", "number", "name"]+columns)
        table_csv.to_csv(filename, index=False)
        print (table_csv)
                ...
                for row in rows:
                    if len(event_details) <= 0:
                        ...
                        for score_row in scores:
                            ...
                            result.append(values)
                        event_details = []
        table_csv = pd.DataFrame(result, columns=['Main event name', name, 'round 1', 'round 2', 'round 3', 'final round', 'total'])
        table_csv.to_csv(filename, index=False)
        print (table_csv)
    ...
    def scrape(self):
        ...
        basketball_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=faede08e-b7b4-4262-bb3d-e5842172ffc5&SetLanguage=en-CA")
        ...
        diving_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=bbd7a72c-8314-44e8-a6d7-80d304b2519a&SetLanguage=en-CA")
        golf_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=417680be-8746-4529-a171-194430dc7371&SetLanguage=en-CA")
        rowing_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=0f976863-1313-4c39-baa2-f9ae01b826f9&SetLanguage=en-CA")
        sailing_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=e0834e86-8a40-4379-b732-e1f17c03ccb1&SetLanguage=en-CA")
        soccer_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=a9254026-0b44-4a0c-b109-20bcba05ad3e&SetLanguage=en-CA")
        softball_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=36c94721-ce80-4323-9d3d-5c46dccb5568&SetLanguage=en-CA")
        swimming_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=346c9aa2-699d-462d-862f-438981700d06&SetLanguage=en-CA")
        tennis_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=f06e0d96-1a2a-43e3-9205-ca04d6b50edb&SetLanguage=en-CA")
        triathlon_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=19a0f7d7-5379-47dd-98aa-35ba06c8dbda&SetLanguage=en-CA")
        volleyball_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=c6a97e4a-b30f-4214-8052-3b0d37483722&SetLanguage=en-CA")
        wrestling_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=6c3b66cb-ea77-4873-93e0-ed84f83b3a36&SetLanguage=en-CA")
        cycling_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=876a594a-2117-494f-9963-385321232a89&SetLanguage=en-CA")
        ...
        athletics_urls = self.get_names_and_urls("https://cg2017.gems.pro/Result/Event_List.aspx?Sport_GUID=f3f4d35b-d99e-4538-a874-eab972650185&SetLanguage=en-CA")
        ...
        self.sports(urls=canoekayak_urls, filename="canoekayak.csv", columns=["time"])
        self.sports(urls=cycling_timed_urls, filename="cycling_timed.csv", columns=["time"])
        self.sports(urls=cycling_points_urls, filename="cycling_points.csv", columns=["points"])
        self.sports(urls=cycling_pos_urls, filename="cycling_pos.csv", columns=["position"])
        self.team_sport(urls=baseball_urls, filename="baseball.csv", columns=["team A","team A runs", "team B", "team B runs"])
        self.team_sport(urls=basketball_urls, filename="basketball.csv", columns= ["team A","team A points", "team B",  "team B points"])
        self.team_sport(urls=soccer_urls, filename="soccer.csv", columns= ["team A","team A score", "team B",  "team B score"])
        self.team_sport(urls=softball_urls, filename="softball.csv", columns= ["team A", "team A runs", "team B", "team B runs"])
        #TENNIS CURRENTLY NOT WORKING
        #self.tennis(urls=tennis_urls[0:2], filename="tennis_individuals.csv", columns=["participant A", "match A", "sets 1 A", "sets 2 A", "sets 3 A" , "participant B", "match B",  "sets 1 B",  "sets 2 B",  "sets 3 B"])
        #self.tennis(urls=tennis_urls[2:], filename="tennis_teams.csv", columns= ["team A", "match A", "sets 1 A", "sets 2 A", "sets 3 A" , "team B",  "match B",  "sets 1 B",  "sets 2 B",  "sets 3 B"])
        ...
        self.team_sport(urls=wrestling_urls[0:2], filename="wrestling_teams.csv", columns=["team A", "team A points", "team B",  "team B points"])
        self.team_sport(urls=wrestling_urls[2:], filename="wrestling_individuals.csv", columns = ["participant A", "participant A points", "participant B",  "participant B points"])
        self.sports(urls=diving_urls, filename="diving.csv", columns = ["points"])
        self.sports(urls=rowing_urls, filename="rowing.csv", columns = ["time"])
        self.sports(urls=sailing_urls, filename="sailing.csv", columns = ["position"])
        self.sports(urls=swimming_urls, filename="swimming.csv", columns = ["time"])
        self.sports(urls=triathlon_urls, filename="triathlon.csv", columns = ["time"])
        self.sports(urls=race_urls, filename="race.csv", columns=["time"])
        self.sports(urls=hurdles_urls,filename="hurdles.csv",columns=["time"])
        self.sports(urls=steeplechase_urls,filename="steeplechase.csv",columns=["time"])
        self.sports(urls=relay_urls,filename="relay.csv",columns=["time"])
        ...
        self.sports(urls=pole_vault_urls,filename="pole_vault.csv",columns=["height"])
        self.sports(urls=shot_put_urls,filename="shot_put.csv",columns=["length"])
        self.sports(urls=discus_urls,filename="discus.csv",columns=["length"])
        self.sports(urls=javelin_urls, filename="javelin.csv", columns=["length"])
        self.sports(urls=hammer_urls,filename="hammer.csv",columns=["length"])
        self.thlons(urls=decathlon_urls,filename="decathlon.csv",columns=["100m", "Long Jump", "Shot Put", "High Jump", "400m", "110m Hurdles", "Discus", "Pole Vault", "Javelin", "1500m", "Points"], type=0)
        self.thlons(urls=heptathlon_urls,filename="heptathlon.csv",columns=["100m Hurdles", "High Jump", "Shot Put", "200m", "Long Jump", "Javelin", "800m", "Points"],type=1)